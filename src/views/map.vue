<template>
  <div class="mx-auto max-w-5xl p-2 text-xs">
    ...
    >
      ...
      <div class="flex items-center justify-center mt-3">
        ...
        <div class="text-gray-500 mr-2">{{ item.current.code }}</div>
        <template v-if="item.mode !== 'ranked'">
          <div class="text-gray-500 mr-2">剩余时间</div>
          <div class="text-gray-500">{{ item.current.remainingTimer }}</div>
        </template>
      </div>

      <div class="flex items-center justify-center mt-3">
        <div class="text-gray-500 mr-2">下张地图</div>
        <div class="text-gray-500 mr-2">{{ item.next.code }}</div>
        <template v-if="item.mode !== 'ranked'">
          <div class="text-gray-500 mr-2">持续时间</div>
          <div class="text-gray-500">{{ item.next.DurationInMinutes }}分钟</div>
        </template>
      </div>
    </div>
  </div>
...
const getMapInfo = async () => {
  ...
  if (data.status === 200) {
    for (let i in data.data) {
      data.data[i].mode = i
      mapInfo.value.push(data.data[i])
    }
  }
}
onMounted(() => {
